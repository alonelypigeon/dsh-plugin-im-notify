// IM/Webhook 扇出：把通知（自动通知 / 外来转发）推送到 IM 渠道。
//
...
// 失败语义：每 2s 重试一轮，积压超过 60s 丢弃（onDrop 告警）。
...
/**
 * IM 重试队列：每 2s 一轮重试、60s 过期丢弃，按「渠道 × 通知」逐条独立投递
 * ——单渠道故障不阻塞其它渠道。push 即刻尝试首轮，失败的条目留在队列里等下一轮。
 */
